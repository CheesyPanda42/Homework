...
class Box:public Shape {
    private:
        ...
        float diag;
    public:
        Box( float c_x, float c_y, float s_x, float s_y ): center_x(0.0f), center_y(0.0f), side_x(0.0f), side_y(0.0f), diag(0.0f)
        {    center_x = c_x;
             center_y = c_y;
             side_x = s_x;
             side_y = s_y;
             diag = sqrt((s_x*s_x) + (s_y*s_y));
        };
...
class Rhombus:public Shape {
    private:
        ...
        float diag_y;
    public:
        Rhombus( float c_x, float c_y, float dia_x, float dia_y ):center_x(0), center_y(0), diag_x(0),diag_y(0)
        {
            center_x = c_x;
            center_y = c_y;
            diag_x = dia_x;
